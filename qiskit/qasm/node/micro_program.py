...
"""Node for an OPENQASM micro_program definition."""

from .node import Node


class MicroProgram(Node):
    """Node for an OPENQASM gate definition.

    children[0] is an id node.
    If len(children) is 3, children[1] is an idlist node,
    and children[2] is a micro_programbody node.
    Otherwise, children[1] is an expressionlist node,
    children[2] is an idlist node, and children[3] is a gate body node.
    """

    def __init__(self, children):
        """Create the micro_program node."""
        super().__init__('micro_program', children, None)
    ...
    def qasm(self, prec=15):
        ...
        string = "micro_program " + self.name
